...
struct pl330_reqcfg {
	...
	/* TODO: all these over. */
	u32      brst_len:5;
	u32      brst_size:3; /* in power of 2 */
	bool     flush_once:true;
	bool     wfp_once:true;
	bool     unroll_loop;
	bool     device_fc;
...
static inline u32 _emit_LDPB(unsigned dry_run, u8 buf[], u8 peri)/* Burst */
{
	if (dry_run)
		return SZ_DMALDP;
...
static inline u32 _emit_LDPS(unsigned dry_run, u8 buf[], u8 peri)/* Single */
{
	if (dry_run)
		return SZ_DMALDP;
...
static inline u32 _emit_LP(unsigned dry_run, u8 buf[], unsigned cnt, u8 loop)/* Always */
{
	if (dry_run)
		return SZ_DMALP;
...
)
{
	...
	bool                 flush_once = preqcfg->flush_once;
	  ...
	  while (cyc--) {
		...
		off += _emit_LDPS(dry_run, &buf[off], peri);
		off += _emit_ST(dry_run, &buf[off]);
	}
	return off;
}
...
)
{
	...
	while (cyc--) {
		...
		off += _emit_LD(dry_run, &buf[off]);
		off += _emit_STPS(dry_run, &buf[off], peri);
	}
	return off;
}
