...
struct dma_pl330_chan {
	...
	struct list_head submitted_list;  // added from 2014.2
...
struct dma_pl330_dmac {
	...
	struct device_dma_parameters dma_parms;// added from 2014.2
	...
	unsigned int num_peripherals;// added from 2014.2
...
static int pl330_control_term_all(struct dma_pl330_chan *pch)
{
	...
	list_splice_tail_init(&pch->submitted_list, &pdmac->desc_pool);// added from 2014.2
...
{
	struct dma_pl330_chan *pch = to_pchan(chan);// added from 2014.2
	unsigned long flags;// added from 2014.2

	spin_lock_irqsave(&pch->lock, flags);// added from 2014.2
	list_splice_tail_init(&pch->submitted_list, &pch->work_list);// added from 2014.2
	spin_unlock_irqrestore(&pch->lock, flags);// added from 2014.2
...
static dma_cookie_t pl330_tx_submit(struct dma_async_tx_descriptor *tx)
{
	...
	while (!list_empty(&last->node)) {
		...
		//list_move_tail(&desc->node, &pch->work_list);
		list_move_tail(&desc->node, &pch->submitted_list);// added from 2014.2
	}

	cookie = dma_cookie_assign(&last->txd);
	list_add_tail(&last->node, &pch->work_list);// added from 2014.2
...
)
{
	...
	caps->residue_granularity = DMA_RESIDUE_GRANULARITY_DESCRIPTOR;//added from 2014.2
...
)
{
	...
	pdmac->num_peripherals = num_chan;// added from 2014.2
	...
	for (i = 0; i < num_chan; i++) {
		...
		INIT_LIST_HEAD(&pch->submitted_list);// added from 2014.2
	...
	adev->dev.dma_parms = &pdmac->dma_parms;// added from 2014.2
